...
- `server/app.js` — Hono app (separated from server entry for testability). `POST /intent` accepts `{ intent, code? }`, calls `createSpec`, returns spec as JSON (201) or error (400)
...
- `server/tests/intent.route.test.js` — route tests for success, empty intent, missing intent
...
- Implement test generation step (Tests phase of S1 loop)
